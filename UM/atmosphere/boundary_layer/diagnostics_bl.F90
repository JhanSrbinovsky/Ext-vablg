...
!CABLE: Lestevens
!, surf_radflux(cable% mp% row_length,cable% mp% rows)                                   &
...
IF (sf(293,3)) THEN
  IF (cable% um%l_cable .or. dim_cs1  ==  4) THEN
    CALL expand_from_mask (                                             &
...
! Items 141-144 Tiled TSOIL layers 1-4
...
! Items 145-148 Tiled SMCL layers 1-4

      do k=1,dsm_levels
!        vcode=144+k
        vcode=140+dsm_levels+k
        IF (SF(vcode,3)) THEN
           ! DEPENDS ON: set_pseudo_list
           CALL SET_PSEUDO_LIST(NTILES,LEN_STLIST,                         &
     &        STLIST(1,STINDEX(1,vcode,3,im_index)),                       &
...
! Items 149-152 Tiled STHF layers 1-4

      do k=1,dsm_levels
!        vcode=148+k
        vcode=140+(2*dsm_levels)+k
        IF (SF(vcode,3)) THEN
           ! DEPENDS ON: set_pseudo_list
           CALL SET_PSEUDO_LIST(NTILES,LEN_STLIST,                         &
     &        STLIST(1,STINDEX(1,vcode,3,im_index)),                       &
...
! Items 153-155 Tiled SNOW_DEPTH3L layers 1-3

      do k=1,3
!        vcode=152+k
        vcode=140+(3*dsm_levels)+k
        IF (SF(vcode,3)) THEN
           ! DEPENDS ON: set_pseudo_list
           CALL SET_PSEUDO_LIST(NTILES,LEN_STLIST,                         &
     &        STLIST(1,STINDEX(1,vcode,3,im_index)),                       &
...
! Items 156-158 Tiled SNOW_MASS3L layers 1-3

      do k=1,3
!        vcode=155+k
        vcode=140+(3*dsm_levels+3)+k
        IF (SF(vcode,3)) THEN
           ! DEPENDS ON: set_pseudo_list
           CALL SET_PSEUDO_LIST(NTILES,LEN_STLIST,                         &
     &        STLIST(1,STINDEX(1,vcode,3,im_index)),                       &
...
! Items 159-161 Tiled SNOW_TMP3L layers 1-3

      do k=1,3
!        vcode=158+k
        vcode=140+(3*dsm_levels+6)+k
        IF (SF(vcode,3)) THEN
           ! DEPENDS ON: set_pseudo_list
           CALL SET_PSEUDO_LIST(NTILES,LEN_STLIST,                         &
     &        STLIST(1,STINDEX(1,vcode,3,im_index)),                       &
...
! Items 162-164 Tiled SNOW_RHO3L layers 1-3

      do k=1,3
!        vcode=161+k
        vcode=140+(3*dsm_levels+9)+k
        IF (SF(vcode,3)) THEN
           ! DEPENDS ON: set_pseudo_list
           CALL SET_PSEUDO_LIST(NTILES,LEN_STLIST,                         &
     &        STLIST(1,STINDEX(1,vcode,3,im_index)),                       &
...
! Items 165 Tiled SNOW_RHO1L 

!      vcode=165
      vcode=140+(3*dsm_levels+12)+1
      IF (SF(vcode,3)) THEN
         ! DEPENDS ON: set_pseudo_list
         CALL SET_PSEUDO_LIST(NTILES,LEN_STLIST,                         &
     &      STLIST(1,STINDEX(1,vcode,3,im_index)),                       &
...
! Items 166 Tiled SNAGE_TILE

!      vcode=166
      vcode=140+(3*dsm_levels+13)+1
      IF (SF(vcode,3)) THEN
         ! DEPENDS ON: set_pseudo_list
         CALL SET_PSEUDO_LIST(NTILES,LEN_STLIST,                         &
     &      STLIST(1,STINDEX(1,vcode,3,im_index)),                       &
