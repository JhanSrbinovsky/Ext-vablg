...
REAL ::                                                           &
  work_3d(tdims%i_end,tdims%j_end,tdims%k_end)                    &
...
print *, "jhan:ni_conv_ctl Section 3"
! ----------------------------------------------------------------------
          ...
          DO j=udims%j_start,udims%j_end
            DO i=udims%i_start,udims%i_end
              ...
              u_incr_diag_conv(i,j,k) = u_incr_diag_conv(i,j,k)          &
                                       + dubydt_pout(i,j,k)* timestep  
            END DO ! i
          END DO ! j
        END DO ! k

! DEPENDS ON: scmoutput
    CALL scmoutput(u_incr_diag_conv,                                &
          ...
          DO j=vdims%j_start,vdims%j_end
            DO i=vdims%i_start,vdims%i_end
              ...
              v_incr_diag_conv(i,j,k) = v_incr_diag_conv(i,j,k)          &
                                       + dvbydt_pout(i,j,k)* timestep  
            END DO ! i
          END DO ! j
        END DO ! k
! DEPENDS ON: scmoutput
    CALL scmoutput(v_incr_diag_conv,                                &
         'dv_conv','V increment convection','m/s',                  &
         t_avg,d_all,default_streams,'',routinename)

!       Stash 5,192
! DEPENDS ON: scmoutput
    CALL scmoutput(bulk_cf_incr_diag_conv,                          &
         'dbcf_conv',                                               &
         'Bulk cloud frac increment convection','Fraction',         &
         t_avg,d_wet,default_streams,'',routinename)

! DEPENDS ON: scmoutput
    CALL scmoutput(cf_liquid_incr_diag_conv,                        &
...
!      Stash 5,194
! DEPENDS ON: scmoutput
    CALL scmoutput(cf_frozen_incr_diag_conv,                        &
...
! DEPENDS ON: scmoutput
    ...
    IF (i_convection_vn == i_convection_vn_6a ) THEN
      ...
      DO j=1,rows
        DO i=1,row_length
          work_2d(i,j)=REAL(conv_type(i,j))
        END DO
      END DO
...
!       Stash 5,267 
! DEPENDS ON: scmoutput 
    CALL scmoutput(deep_cfl_limited,                                & 
         'cfl_limited_deep','Indicator for CFL limited deep',       & 
         'Indicator',                                               & 
         t_inst,d_point,default_streams,'',routinename)

!       Stash 5,268 
! DEPENDS ON: scmoutput 
    CALL scmoutput(mid_cfl_limited,                                 & 
         'cfl_limited_deep','Indicator for CFL limited mid',        & 
         'Indicator',                                               & 
         t_inst,d_point,default_streams,'',routinename)

!       Stash 5,269
! DEPENDS ON: scmoutput
    CALL scmoutput(deep_ind,                                        &
         'ind_deep','Indicator for deep convection',                &
         'Indicator',                                               &
         t_inst,d_point,default_streams,'',routinename)
...
! DEPENDS ON: scmoutput
    CALL scmoutput(work_2d,                                         &
         'cct','Convective Cloud Top Level of '//                   &
         'Highest Convective Layer','-',                            &
         t_avg,d_sl,default_streams,'',routinename)

    work_2d = REAL(ccb)
! DEPENDS ON: scmoutput
    CALL scmoutput(work_2d,                                         &
         'ccb','Convective Cloud Base Level of '//                  &
         'Highest Convective Layer','-',                            &
         t_avg,d_sl,default_streams,'',routinename)
...
! DEPENDS ON: scmoutput
    CALL scmoutput(work_2d,                                         &
         'lcbase','Convective Cloud Base Level of '//               &
         'Lowest Convective Layer','-',                             &
         t_avg,d_sl,default_streams,'',routinename)

    work_2d = REAL(lctop)
! DEPENDS ON: scmoutput
    CALL scmoutput(work_2d,                                         &
         'lctop','Convective Cloud Top Level of '//                 &
         'Lowest Convective Layer','-',                             &
         t_avg,d_sl,default_streams,'',routinename)

! DEPENDS ON: scmoutput
    CALL scmoutput(lcca,                                            &
         'lcca','Convective Cloud Amount at base of Lowest '//      &
...
! DEPENDS ON: scmoutput
  ...
  ! to why a 0.5 cap should be found.
    DO j=1, rows
      DO i=1, row_length
        work_2d(i,j) = MIN(0.5, cca_2d(i,j))
      END DO
    END DO
...
! DEPENDS ON: scmoutput
    CALL scmoutput(past_precip,                                     &
         'past_precip','History of convective precip',              &
         'kg/m2/s',                                                 &
         t_avg,d_sl,default_streams,'',routinename)


! DEPENDS ON: scmoutput
    CALL scmoutput(past_conv_ht,                                    &
         'past_conv_ht','History of convective depth',              &
         'm', t_avg,d_sl,default_streams,'',routinename)

  END IF ! L_SCMDiags(SCMDiag_conv)
#endif

print *, "jhan:ni_conv_ctl 13"
!----------------------------------------------------------------------------
